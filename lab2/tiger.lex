    ...
    if (i < len - 2 && str[i] == '\\') {
      if (str[i+1] == 'n') {
        ...
        if(str[i+2]>='A'&&str[i+2]<='Z')
        {
          ...
          EM_error(charPos,"Invalid \\^c");
          i+=3;
        }
      } else if (str[i+1] >= '0' && str[i+1] <= '9') {
        if(str[i+2] >= '0' && str[i+2] <= '9'&&str[i+3] >= '0' && str[i+3] <= '9')
        {
...
<INITIAL>"*/" {adjust();  EM_error(charPos,"//* is required");}
...
<COMMENT><<EOF>> {adjust(); yyterminate();EM_error(charPos, "Expect */ at the end of a comment");}
<COMMENT>. {adjust();}
