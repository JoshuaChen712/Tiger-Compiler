...
Temp_tempList F_calleeSavedRegisters(){
    ...
    if(!F_calleeSaved) 
	F_calleeSaved = Temp_TempList(F_RBX(), Temp_TempList(F_R12(), Temp_TempList(F_R13(), 
...
Temp_tempList F_callerSavedRegisters(){
    if(!F_callerSaved)  
	F_callerSaved = Temp_TempList(F_RV(), Temp_TempList(F_RCX(), Temp_TempList(F_RDX(), Temp_TempList(F_R8(), 
...
T_stm F_procEntryExit1(F_frame frame, T_stm stm) {
    ...
    for(Temp_tempList p = calleeSaved; p; p = p->tail){
        ...
        if(!save_pos) save_pos = save_pos_l = Temp_TempList(save, NULL);
        else save_pos_l = save_pos_l->tail = Temp_TempList(save, NULL);
...
AS_instrList F_procEntryExit2(AS_instrList body) {
    Temp_tempList returnSink = Temp_TempList(F_RV(), Temp_TempList(F_SP(), Temp_TempList(F_FP(), F_calleeSavedRegisters())));
...
AS_proc F_procEntryExit3(F_frame frame, AS_instrList body){
    ...
    AS_instr pro1 = AS_Oper(String("pushq `s0"), NULL, Temp_TempList(F_FP(), NULL), NULL);
    AS_instr pro2 = AS_Oper(String("movq `s0, `d0"), Temp_TempList(F_FP(), NULL), Temp_TempList(F_SP(), NULL), NULL);
    sprintf(buffer, "subq $%d, `s0", fsize);
    AS_instr pro3 = AS_Oper(String(buffer), Temp_TempList(F_SP(), NULL), Temp_TempList(F_SP(), NULL), NULL);
