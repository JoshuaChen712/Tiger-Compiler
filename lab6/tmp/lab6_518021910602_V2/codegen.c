...
static Temp_tempList munchArgs(int i, T_expList args){
	...
	emit(AS_Move(String("movq `s0, `d0"), Temp_TempList(dest_reg, NULL), Temp_TempList(arg_reg,NULL)));
	if(i < 5) return Temp_TempList(dest_reg, munchArgs(i+1, args->tail));
	else{
		...
		for(p = reversedArgs; p; p = p->tail){
			//enter stack
			emit(AS_Oper(String("pushq `s0"), NULL, Temp_TempList(arg_reg,NULL), NULL));
		}
		return Temp_TempList(dest_reg, NULL);
	}
}
	...
	switch(e->kind){
		case T_MEM:{
			if(e->u.MEM->kind == T_BINOP && e->u.MEM->u.BINOP.op == T_plus && 
				...
				emit(AS_Oper(String(buffer), Temp_TempList(result, NULL), Temp_TempList(offset, NULL), NULL));
                return result;
			}
				...
				emit(AS_Oper(String(buffer), Temp_TempList(const_reg, NULL), NULL, NULL));
				sprintf(buffer, "movq (`s0), `d0");
				emit(AS_Oper(String(buffer), Temp_TempList(result, NULL), Temp_TempList(const_reg, NULL), NULL));
				return result;
			}
			else if(e->u.MEM->kind == T_TEMP){
				Temp_temp result = Temp_newtemp();
				sprintf(buffer, "movq (`s0), `d0");
				emit(AS_Oper(String(buffer), Temp_TempList(result, NULL), Temp_TempList(e->u.TEMP, NULL), NULL));
				return result;
			}
			else{
				Temp_temp result = Temp_newtemp();
				Temp_temp add = munchExp(e->u.MEM);
				emit(AS_Oper(String("movq (`s0), `d0"), Temp_TempList(result, NULL), Temp_TempList(add, NULL), NULL));
                return result;
			}
		}
		case T_BINOP:{
			...
			if(result != left_op){
                if(e->u.BINOP.op!=T_div){
                    if(e1->kind != T_CONST) emit(AS_Move("movq `s0, `d0", Temp_TempList(result, NULL), Temp_TempList(left_op, NULL)));
                    else{
                        sprintf(buffer, "movq $%d, `d0", e1->u.CONST);
                        emit(AS_Oper(String(buffer), Temp_TempList(result, NULL), NULL, NULL));
                    }
                }
			}
			if(e2->kind == T_CONST){
                if(e->u.BINOP.op!=T_div){
                    ...
                    if(e->u.BINOP.op!=T_div) emit(AS_Oper(String(buffer), Temp_TempList(result, NULL), Temp_TempList(result, NULL), NULL));
                    return result;
                }
			}
			...
			if(e->u.BINOP.op != T_div){
				emit(AS_Oper(String(buffer), Temp_TempList(result, NULL), Temp_TempList(right_op, Temp_TempList(result, NULL)), NULL));
			}
			else{
				//special division for x86-64
				Temp_temp rax = F_RV();
				Temp_temp rdx = F_RDX();
				emit(AS_Move(String("movq `s0, `d0"), Temp_TempList(rax, NULL), Temp_TempList(left_op, NULL)));
				emit(AS_Oper(String("xorq `s0, `d0"), Temp_TempList(rdx, NULL), Temp_TempList(rdx, NULL), NULL));
				emit(AS_Oper(String("idivq `s0"), Temp_TempList(rdx, Temp_TempList(rax, NULL)), Temp_TempList(right_op, Temp_TempList(rax, NULL)), NULL));
				emit(AS_Move(String("movq `s0, `d0"), Temp_TempList(result, NULL), Temp_TempList(rax, NULL)));
			}
			return result;
		}
        ...
        case T_NAME:{
			...
			emit(AS_Oper(as, Temp_TempList(r, NULL), NULL, NULL));
            return r;
		}
		default: assert(0);
	}
...
static void munchStm(T_stm s){
	switch(s->kind){
		case T_MOVE: {
			...
			if(dst->kind == T_MEM){
				if(dst->u.MEM->kind == T_BINOP && dst->u.MEM->u.BINOP.op == T_plus && 
					...
					emit(AS_Oper(String(buffer), NULL, Temp_TempList(r_src, Temp_TempList(dst_r, NULL)), NULL));
					return;
				}
					...
					emit(AS_Oper(String(buffer), NULL, Temp_TempList(r_src, Temp_TempList(dst_l, NULL)), NULL));
					return;
				}
				else if(src->kind == T_MEM){//3
					Temp_temp r_dst = munchExp(dst->u.MEM), r_src = munchExp(src->u.MEM);
					Temp_temp r = Temp_newtemp();
					emit(AS_Oper(String("movq (`s0), `d0"), Temp_TempList(r,NULL), Temp_TempList(r_src, NULL), NULL));
                    //fix a bug here. 1225
					emit(AS_Oper(String("movq `s0, (`s1)"), NULL, Temp_TempList(r,Temp_TempList(r_dst, NULL)), NULL));
					return;
				}
				else{//2
					Temp_temp r_dst = munchExp(dst->u.MEM), r_src = munchExp(src);
					emit(AS_Oper(String("movq `s0, (`s1)"), NULL, Temp_TempList(r_src, Temp_TempList(r_dst, NULL)), NULL));
					return;
				}
			}
				...
				emit(AS_Move(String("movq `s0, `d0"), Temp_TempList(dst->u.TEMP,NULL), Temp_TempList(r_src, NULL)));
				return;
			}
			else assert(0);
		}
		...
		case T_CJUMP: {
            ...
            emit(AS_Oper(String(buffer), NULL, Temp_TempList(r_l, Temp_TempList(r_r, NULL)), NULL));
...
AS_instrList F_codegen(F_frame f, T_stmList stmList) {
    ...
    Temp_tempList l = Temp_TempList(F_RDI(), 
	                  Temp_TempList(F_RSI(), 
					  Temp_TempList(F_RDX(), 
					  Temp_TempList(F_RCX(), 
					  Temp_TempList(F_R8(), 
    ...
    for(int i = 0; i < 6 && p; i++, p = p->tail){
        if(p->head->kind == inReg) 
		   emit(AS_Oper(String(""), NULL, Temp_TempList(p->head->u.reg, l->tail), NULL));
        l = l->tail;
    }
